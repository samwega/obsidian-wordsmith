...
export class ContextControlPanel extends ItemView {
    ...
    async onOpen() {
        ...
        const titleEl = container.createEl("h3", { text: "Text Transform" });
        titleEl.style.marginTop = "0px"; // Attempt to reduce top margin
        titleEl.style.marginBottom = "2px"; // Small space before subtitle

        // Subtitle
        const subTitleEl = container.createEl("h5", { text: "AI Context Options" });
        subTitleEl.style.marginTop = "0px";
        subTitleEl.style.marginBottom = "15px"; // Space before the first setting
        subTitleEl.style.color = "var(--text-muted)"; // Style subtitle like a typical description
